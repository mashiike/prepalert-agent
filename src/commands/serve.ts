...
function initializeServeContext(project: Project, opts: ServeCommandOptions): ServeContext {
  ...
  const { key: exportSecret, autoGenerated: exportAutoGen } = resolveExportSecret(serve?.exportSecret);
  if (exportAutoGen) {
  ...
  if (serve?.auth) {
    const { key: sessionSecret, autoGenerated: sessionAutoGen } = resolveSessionSecret(serve.sessionSecret);
    if (sessionAutoGen) {
