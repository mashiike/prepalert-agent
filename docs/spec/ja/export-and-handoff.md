...
各セッションには `/sessions/{sessionId}` のパーマリンク URL でアクセスできる。SPA がクライアントサイドルーティングでセッション詳細を表示する。

- ブラウザの URL バーにセッション ID が反映され、ブックマークや共有が可能
- SPA の「Copy URL」ボタンでパーマリンクをクリップボードにコピーできる
- `serve` モードでは `serve.baseUrl`（または自動検出）を使ったフル URL がシステムプロンプトに含まれ、Agent が Slack 等にリンクを投稿する際に使用できる
- `run` モードではセッション ID のみがシステムプロンプトに含まれる

`serve.auth` が設定されている場合、パーマリンクへのアクセスには OIDC 認証が必要。
...
**重要:** `serve.auth`（OIDC）を設定すると `/api/*` エンドポイントおよび SPA に認証がかかる（詳細は [auth.md](./auth.md)）。未設定の場合は認証なしになる。セッションデータにはインシデント調査ログ（ホスト名、内部 IP、スタックトレース等）が含まれ得るため、`serve.auth` を設定するか、ネットワーク層（VPC, IAP, ALB の認証等）で保護すること。
